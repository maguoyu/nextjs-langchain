...
const defaultMenus: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { name: '角色管理', path: '/dashboard/roles', icon: 'Shield' },
      { name: '权限管理', path: '/dashboard/permissions', icon: 'Key' },
      { name: '菜单管理', path: '/dashboard/menus', icon: 'Menu' },
    ],
  },
]
...
function BotIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M15 14a1 1 0 100 2 1 1 0 000-2z" />
      <path d="M9 10h6" />
    </svg>
  )
}
...
export function Sidebar() {
  ...
  const renderIcon = (icon: string) => {
    ...
    const icons: Record<string, React.ReactElement> = {
      Dashboard: <DashboardIcon className={size} />,
      Bot: <BotIcon className={size} />,
      Settings: <SettingsIcon className={size} />,
      Users: <UsersIcon className={size} />,
      Shield: <ShieldIcon className={size} />,
      Key: <KeyIcon className={size} />,
      Menu: <MenuIcon className={size} />,
    }
    return icons[icon] || (
      <span className="w-5 h-5 flex items-center justify-center rounded-md bg-gray-100 dark:bg-gray-700 text-xs font-bold text-gray-500 dark:text-gray-300">
        {icon.charAt(0)}
      </span>
    )
  }
  ...
  const isChildActive = (children?: MenuItem[]) =>
    children?.some((c) => pathname === c.path) ?? false
  ...
  return (
    <aside
      ...
      style={{
        background: 'linear-gradient(180deg, #1e3a5f 0%, #1a2e4a 50%, #152238 100%)',
      }}
    >
      {/* Logo Area */}
            ...
            <div key={menu.path}>
              {menu.children ? (
                  ...
                  <div className="space-y-0.5">
                    {menu.children.map((child) => (
                      ...
                      >
                        ...
                        <span className={`shrink-0 ${isActive(child.path) ? 'text-white' : 'group-hover:text-white/80'}`}>
                ...
                >
                  ...
                  <span className={`shrink-0 ${isActive(menu.path) ? 'text-white' : 'group-hover:text-white/80'}`}>
